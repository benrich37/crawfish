...
def test_get_gauss_smear_spectrum():
    ...
    erange = np.arange(0, 10, 0.1, dtype=np.float32)
    e_sabcj = np.array([5], dtype=np.float32)
    w_sabcj = np.array([1], dtype=np.float32)
...
def test_get_uneven_integrated_array():
    ...
    e_sabcj = np.array([1, 2], dtype=np.float32)
    w_sabcj = np.array([1, 1], dtype=np.float32)
...
def test_get_lti_spectrum():
    ...
    e_sabcj = np.ones(9, dtype=np.float32) * 0.5
    e_sabcj = e_sabcj.reshape([1, 3, 3, 1, 1])
    e_sabcj[:, 1:, :, :, :] += 0.2
    e_sabcj[:, :, 1:, :, :] += 0.2
    e_sabcj[:, 2, :, :, :] += 0.2
    e_sabcj[:, :, 2, :, :] += 0.2
    w_sabcj = np.ones(9, dtype=np.float32)
