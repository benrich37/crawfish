...
def get_h_uu_p_uu_s_uu(proj_tju: np.ndarray[COMPLEX_DTYPE], e_tj: np.ndarray[REAL_DTYPE], occ_tj: np.ndarray[REAL_DTYPE], wk_t: np.ndarray[REAL_DTYPE]):
...
def _get_gen_tj(
    ...
    wk_cw_tj_uu = np.real(np.conj(proj_tju)[:,:,np.newaxis,:] * proj_tju[:,:,:,np.newaxis] * wk_t[:,np.newaxis,np.newaxis,np.newaxis])
    orbs = np.asarray(list(orbs_u) + list(orbs_v))
    t,j,u=np.shape(proj_tju)
    gen_tj = np.zeros([t,j], dtype=REAL_DTYPE)
    return _get_gen_tj_jit(gen_tj, gen_uu, wk_cw_tj_uu,orbs)
...
def _get_gen_tj_jit(gen_tj, gen_uu, wk_cw_tj_uu, orbs):
    for u in orbs:
        for v in orbs:
            if not u == v:
                gen_tj += h_uu[u,v]*wk_cw_tj_uu[:,:,u,v]
    return gen_tj
...
def get_pdos_tj(proj_tju: np.ndarray[COMPLEX_DTYPE], orbs: list[int]) -> np.ndarray[REAL_DTYPE]:
